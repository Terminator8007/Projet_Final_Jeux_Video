...
func physics_update(delta: float) -> void:
	...
	if last_direction.x > 0.7:
		anim_player.play("Attaque1_coter")
		player.sprite_attaque.flip_h = false
	elif last_direction.x < -0.7:
		anim_player.play("Attaque1_coter")
		player.sprite_attaque.flip_h = true
	elif last_direction.y < -0.7:
		anim_player.play("Attaque1_haut")
	elif last_direction.y > 0.7:
		anim_player.play("Attaque1_bas")
...
func _on_animation_player_animation_finished(anim_name: StringName) -> void:
	if anim_name == "Attaque1_coter" or anim_name == "Attaque1_haut" or anim_name == "Attaque1_bas":
		if other_attack:
			other_attack = false
			damage_area_colider.disabled  = true
			damage_area_coliding = false
			Transitioned.emit(self, "Attaque2", last_direction)
		else:
			damage_area_colider.disabled  = true
			damage_area_coliding = false
